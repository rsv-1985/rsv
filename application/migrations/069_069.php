...
class Migration_069 extends CI_Migration
{
    ...
    public function up()
    {
        ...
        $this->db->query("UPDATE `ax_customer` SET `phone` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(phone,' ',''),'(',''),')',''),'-',''),'+','')");
        $this->db->query("UPDATE `ax_order` SET `telephone` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''),'(',''),')',''),'-',''),'+','')");
        $this->db->query("UPDATE `ax_vin` SET `telephone` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''),'(',''),')',''),'-',''),'+','')");
        $this->db->query("UPDATE `ax_waybill_parcel` SET `telephone` = REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(telephone,' ',''),'(',''),')',''),'-',''),'+','')");
