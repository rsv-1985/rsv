                ...
                <div class="box-body">
                    <div class="form-group">
                        ...
                        <input required type="text" class="form-control" name="name"
                               value="<?php echo set_value('name', $supplier['name']); ?>" maxlength="250">
                    </div><!-- /.form group -->
                    <div class="form-group">
                    ...
                    <div class="checkbox">
                        <label>
                            <input type="checkbox" name="stock"
                                   value="1" <?php echo set_checkbox('stock', true, (bool)$supplier['stock']); ?>> <?php echo lang('text_stock'); ?>
                        </label>
                    </div>
                    <hr>
                    ...
                    <?php if ($count > 0) { ?>
                        <a href="/autoxadmin/supplier/delete_products/<?php echo $supplier['id']; ?>"
                           id="delete-product"
                           class="btn btn-danger pull-right"><?php echo lang('button_delete_products'); ?></a>
                    <?php } ?>
                </div><!-- /.box-body -->
            </div>
        </div>
            ...
            <div class="box">
                ...
                <div class="box-body">
                        ...
                        <?php if (!empty($pricing)) { ?>
                            ...
                            <?php foreach ($pricing as $price) { ?>
                                <tr id="row<?php echo $q; ?>">
                                    ...
                                    <td>
                                        <select name="pricing[<?php echo $q; ?>][customer_group_id]" class="form-control">
                                            <option></option>
                                            <?php foreach ($groups as $group){?>
                                                <option value="<?php echo $group['id'];?>" <?php if($group['id'] == $price['customer_group_id']){?>selected<?php } ?>><?php echo $group['name'];?></option>
                                            <?php } ?>
                                        </select>
                                    </td>
                                    <td>
                                        <select name="pricing[<?php echo $q; ?>][method_price]" class="form-control">
                                            <option value="+"
                                                    <?php if ($price['method_price'] == '+'){ ?>selected<?php } ?>><?php echo lang('text_pricing_margin'); ?></option>
                                            <option value="-"
                                                    <?php if ($price['method_price'] == '-'){ ?>selected<?php } ?>><?php echo lang('text_pricing_discount'); ?></option>
                                        </select>
                                    </td>
                    ...
                    <a href="#" class="btn btn-info pull-right"
                       onclick="add_row(event);"><?php echo lang('button_add'); ?></a>
                </div><!-- /.box-body -->
            </div>
        </div>
        <div class="col-md-12">
...
<script>
    ...
    function add_row(event) {
        ...
        html += '<td>';
        html += '<select name="pricing[' + row + '][customer_group_id]" class="form-control">';
        html += '<option></option>';
        <?php foreach ($groups as $group){?>
        html += '<option value="<?php echo $group['id'];?>"><?php echo $group['name'];?></option>';
        <?php } ?>
        html += '</select>';
        html += '</td>';
        html += '<td>';
        ...
        html += '<td><input type="text" name="pricing[' + row + '][fix_value]" class="form-control"></td>';
        html += '<td>';
