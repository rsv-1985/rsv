                            ...
                            <?php if($tree->Level == 1){?>
                                <li id="<?php echo $tree->ID_tree;?>">
                                    <?php if($tree->Childs > 0){?>
                                        <i class="fa fa-plus-square-o"></i> <a href="#" onclick="show_tree('<?php echo $tree->ID_tree;?>')"><?php echo $tree->Name;?></a>
                                    <?php }else{?>
                                        <i class="fa fa-circle-o"></i> <a href="<?php echo current_url();?>?id_tree=<?php echo $tree->ID_tree;?>"><?php echo $tree->Name;?></a>
                                    <?php } ?>
                                </li>
                            <?php } ?>
                        <?php } ?>
                    ...
                    <?php foreach($parts as $part){?>
                            <?php if($part->available['products'] || $part->available['cross']){?>
                                <?php if($part->available['products']){?>
                                    ...
                                    <tr>
                                        <td>
                                            <img src="/image?img=<?php echo $part->Preview;?>&width=50" title="<?php echo $part->Brand;?>" class="img-responsive">
                                        </td>
                                        <td>
                                            <a target="_blank" href="/product/<?php echo $product['slug'];?>"><?php echo $product['sku'];?></a>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            ...
                                            <small><a href="/cart" id="<?php echo md5($product['slug']);?>"
                                                <?php if(!key_exists(md5($product['slug']),$this->cart->contents())){?>
                                                    style="display: none;"
                                                <?php } ?>
                                            ><i class="fa fa-shopping-cart"></i> <?php echo lang('text_in_cart');?></a></small>
                                        </td>
                                    </tr>
                                    <?php } ?>
                                <?php } ?>
